      ...
      <div className="mx-auto max-w-8xl px-5 sm:px-6 lg:px-10">
        <Reveal className="mb-10 sm:mb-12">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
              <h2 className="text-[clamp(1.75rem,6vw,3.5rem)] font-bold text-ink leading-[1.1] tracking-tight text-balance">Ervaringen van onze patiënten.</h2>
              <p className="mt-3 text-ink-muted">Persoonlijke aandacht en goede zorg staan centraal.</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">{[1, 2, 3, 4, 5].map((i) => <Star key={i} size={16} fill="#168FE3" className="text-bcn-blue" />)}</div>
              <div className="text-sm"><span className="font-bold text-ink">{companyInfo.googleRating}</span><span className="text-ink-muted"> op Google</span></div>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal className="mt-10">
          <LinkButton to="/reviews" variant="secondary" withArrow>Bekijk alle ervaringen</LinkButton>
        </Reveal>
      </div>
    </section>
  );
